	...
	if (data->opcode == AFE_PORT_CMDRSP_GET_PARAM_V2 ||
		...
		if (data->opcode == APR_BASIC_RSP_RESULT) {
			...
			if (payload[1] != 0) {
				...
				/* unsupported/bad-param is an expected reply */
				if (payload[1] == ADSP_EBADPARAM ||
				    payload[1] == ADSP_EUNSUPPORTED)
	...
	if (ret > 0) {
		if (wait) {
			...
			if (!ret) {
				...
				int st = atomic_read(&this_afe.status);
				/*
				 * ADSP_EBADPARAM/ADSP_EUNSUPPORTED on optional
				 * set/get-param requests is an expected "this port
				 * has no such param" answer, not a real fault.
				 */
				if (st == ADSP_EBADPARAM || st == ADSP_EUNSUPPORTED)
					pr_debug("%s: DSP not supported[%s]\n",
						 __func__,
...
static int afe_send_port_topology_id(u16 port_id)
{
	...
	if (ret) {
		/*
		 * ADSP_EBADPARAM (-EINVAL) means this port has no per-port
		 * topology (common for plain MI2S RX ports). That is a normal
		 * "not supported" result, not a failure worth a KERN_ERR.
		 */
		pr_debug("%s: AFE set topology id for port 0x%x not supported %d\n",
			__func__, port_id, ret);
		goto done;
	}
