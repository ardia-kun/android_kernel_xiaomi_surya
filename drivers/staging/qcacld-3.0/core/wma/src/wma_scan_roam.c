				 ...
				 uint32_t len)
{
	 ...
	 * Use actual TLV counts from param_buf for variable-length arrays
	 * instead of num_tlv, since firmware may send fewer entries for
	 * result/neighbor arrays than trigger arrays (e.g., 0 results when
	 * no roam occurred).
	 */
	...
	if (rem_len < (param_buf->num_roam_result *
		       sizeof(wmi_roam_result))) {
		wma_err_rl("Invalid roam result data");
		goto err;
	}

	rem_len -= param_buf->num_roam_result * sizeof(wmi_roam_result);
	if (rem_len < (param_buf->num_roam_neighbor_report_info *
		       sizeof(wmi_roam_neighbor_report_info))) {
		wma_err_rl("Invalid roam neighbor report data");
		goto err;
	}

	rem_len -= param_buf->num_roam_neighbor_report_info *
		   sizeof(wmi_roam_neighbor_report_info);
