...
static ssize_t gov_debug_show(struct kobject *kobj, struct kobj_attribute *attr,
			  char *buf)
{
	return sprintf(buf, "%d\n", governor_debug);
}

static ssize_t gov_debug_store(struct kobject *kobj, struct kobj_attribute *attr,
			   const char *buf, size_t count)
{
	int ret;
	bool value;
...
static struct kobj_attribute gov_debug_attr = __ATTR(debug, 0664, gov_debug_show, gov_debug_store);
