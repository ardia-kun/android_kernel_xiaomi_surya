...
{
	if (!SCHED_FEAT_ENFORCE_ELIGIBILITY)
		return 1;
...
static struct sched_entity *pick_eevdf(struct cfs_rq *cfs_rq)
{
	 ...
	 */
	if (SCHED_FEAT_RUN_TO_PARITY && curr && curr->vlag == curr->deadline)
		return curr;
...
static inline u64 scale_slice(u64 delta, struct sched_entity *se) {
	u64 res = mul_u64_u32_shr(delta, sched_prio_to_wmult[se->burst_score], 22);
	return max_t(u64, res, 500000ULL);
}
...
#ifdef CONFIG_SCHED_BORE
	if (sched_bore)
		se->slice = scale_slice(se->slice, se);
#endif
...
place_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	 ...
	 */
	if (SCHED_FEAT_PLACE_LAG && cfs_rq->nr_running) {
		struct sched_entity *curr = cfs_rq->curr;
		unsigned long load;
	 ...
	 */
	if (SCHED_FEAT_PLACE_DEADLINE_INITIAL && (flags & ENQUEUE_INITIAL))
		vslice /= 2;
				   ...
				   struct find_best_target_env *fbt_env)
{
	...
	*backup_cpu = -1;
	...
	/* fast path for prev_cpu (kerneltoast/Sultan-inspired EAS bypass) */
	if ((capacity_orig_of(prev_cpu) == capacity_orig_of(cpu)) &&
		!cpu_isolated(prev_cpu) && cpu_online(prev_cpu) &&
		idle_cpu(prev_cpu)) {

		/*
		 * If prev_cpu is already idle and has enough capacity, reuse it
		 * immediately to skip expensive energy calculations.
		 */
		target_cpu = -1;
